...
function buildReviewEmailHtml(clientName, furkidName, serviceName, isKinderPuppy) {
  ...
  return '<div style="font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f1f5f9;">'
    ...
    + '<p style="font-size: 15px; color: #334155; line-height: 1.6; margin: 0 0 16px 0;">What a journey it has been! &#128062; We are so proud of <strong>' + furkidName + '</strong> and the incredible progress made throughout the <strong>' + serviceName + '</strong>.</p>'
    ...
    + '<p style="font-size: 14px; color: #78350f; line-height: 1.6; margin: 0 0 20px 0;">If you\'ve enjoyed training with us, a Google review would mean the world to our small team. It takes less than a minute and helps other furkid families find us!</p>'
...
Deno.serve(async (req) => {
  try {
    ...
    const todayStr = sgNow.toISOString().split('T')[0];
    ...
    for (const booking of confirmedBookings) {
      ...
      await base44.asServiceRole.entities.Booking.update(booking.id, { booking_status: 'completed' });
      completedCount++;

      const clientEmails = getClientEmails(booking);
      if (clientEmails.length === 0) {
        console.log('No email found for booking ' + booking.id + ', skipping review email');
        continue;
      }
